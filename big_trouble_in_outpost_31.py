...
# v0.23.0
# Continuing major update to audio
# Added sounds for dogs
# Added audio for The Thing being killed
# Improved flamethrower sounds
# Added separate sounds for arm and leg hits
# Added sound for The THing transforming and consuming

# v0.23.1
# Dramatically decreased the rendering speed by reducing the number of times the screen renders per turn

# v0.23.2
# Added a score counter to the in-game screen
# Projectile weapons now have an unlimited range for targets, although it's not guaranteed they will be able to actually reach that target
# No longer allows for duplicate scientists -- can only be one scientist of the same name

# v0.23.2
# Last update broke the attacking decision system due to labelling all objects as melee weapons.  This has been fixed.

# v0.23.2
# Major issue now in the way that The Thing makes copies of scientists.  Game is lagging and hanging the more scientist copies you create.  Fix in progress.

# v0.23.3
# Fix to previous issue is to large to correct in short time frame.  As alternate fix, you now have access to all character transformations from the beginning.
# Added even more sound effects for different materials and objects.

# v0.23.4
# Added even more sound effects for different materials and objects.

# v0.23.5
# Added rifles and shotguns

# v0.23.6
# Added option menu for selecting lab material

# v0.23.7
# Gave Nauls an axe
# Added a gun timer so enemies cannot rapidfire
# Added scalpel and axe sound effects
# Added childs, blair, and copper -- with unique weapons each

# v0.23.7
# Major update to how units look for and decide which weapon is most powerful.  Seems to be a good improvement to the AI -- also scientists and smarter and more dangerous now

# v0.23.8
# The 10 unique scientists now have their own character-specific colours
# Removed the inventory weight limitation
# MacReady and other characters now throw bottles of J&B at you

# v0.23.9
# Removed the entire left click menu.  Everything is now handled through keyboard shortcuts
# Added Windows to Very Hard difficulty.  He is now the one with the Flamethrower, MacReady exclusively throws J&B bottles.
# Fixed the 'F' hotkey pickup system.  Can now take objects from inventory again, like lockers etc.
# Changed the 8-bit explosion sound to be more natural and fit with the other effects of the game.
# Game will now only generate a message in the message panel if it involves a characer unit.  This will remove a lot of the extranous messages, like "Brick collides with wall" for example after an explosion.

# v0.24.0
# Removed the examining and suspicion of scientists of each other in the lab.  Should lead to less instances of scientists killing each other before you even get inside.  They might still kill each other if they get in one another's way in a doorway or something.  But that's natural/
# Characters will now properly freak out when they encounter The Thing when it has taken their own form.
# When scientists encounter The Thing copying someone who has died, they will also freak out.

# v0.24.1
# Total overhaul to the scoreboard.  Now has special score values for every unique character.
# Now displays a final score whenever you die or quit the game.

# v0.24.2
# Added menu showing how many units are remaining.  Press 'U'
# Added game victory condition complete with victory music.

# v0.24.3
# Added locked content.  Weapons as well as bonuses like the ablity to change game skin and remove the FOV will require certain game scores to unlock.
# Added 'K' hotkey to toggle the rendering of mouseover tooltips on or off
# Changed the Rifle to a Sniper Rifle and extended the range and accuracy
# Added automatic character naming if you don't enter a name yourself

# v0.24.4
# Added Invincibility and "Real-Time' game mode to unloackable cheats.
# Disabled score recording while using cheats
# Added a couple of special character names that will enable easter eggs
# Fixed player colour back to purple again

# v0.24.5
# Implemented the real-time game mode.  Will now be the default game mode and not unlockable
# Fixed that units would not throw some objects properly
# Fixed that units' basic punch attack would do no damage.  Turns out because the game didn't register their arm's "inernal" energy bonus as anything useful.  Oops.

# v0.24.6
# Added computer sound effects
# Added menu sound effects
# Added throwing sound effects

# v0.9
# Removed and updated out-of-date backstory from main menu
# Removed all the debug info from the game display panels
# Added controls menu (press K)
# Game is now considered functionally complete!  All that remains is to add a few more objects & dialogue for flavour, prepare the game EXE, physical package materials, and do final testing!

# v1.0
# Changed unique characters so they will no longer spawn outside the lab
# Added Kalo
# Added Louis Cole
# Added the dining room
# Adjusted the balance on the unlockables for points

# v1.01
# Fixed that the scoreboard was not calculating game bonuses until after the final scoreboard had been shown.
# Implemented a system where if a unit attacks another unit, and a unit friendly to the target is nearby, they will get upset and attack the aggressive unit.  This change was necessary to fix the issue where if you disguised yourself as a scientist, only the target you are attacking would become aggressive against you.  Other neaby units would basicaly ignore what was happening, making the game too easy )and unrealistic).  
# Implemented a system that keeps track of who takes credit for a kill.  If units in the lab kill one another, those kills now only count for 50% point value.  

# v1.02
# Fixed that flashlight could not be equipped since it was not flagged as a melee weapons
# Fixed that doors would not open if the wall was within 1 tile of the edge of the screen
# Fixed that unlockables would not unlock properly
# Reduced point value for kills that hapen with no involvement from the player
...
# [DONE] # Game needs to render faster
# [DONE] Need a "best round" score on the board
# [DONE]sound effects overhaul
# [DONE] Generator kills indoor sounds
# [DONE] target is too far away with rocket launcher
# [DONE] No duplicate scientists
# [DONE] need a score tracker while playing
# [FEATURE NOT BEING ADDED]MacReady does not seem to mind when copy of himself is in the room
# [FEATURE NOT BEING ADDED]Should units pick up and throw objects?  Too much for initial release?
# [FEATURE NOT BEING ADDED]Locked doors for gun room?  retinal scanner?
# [FEATURE NOT BEING ADDED]Unlockables
# [FEATURE NOT BEING ADDED]macready shold throw JandB
# [FEATURE NOT BEING ADDED]lockers spawn in front of doors
# [DONE] impact sound needs improving
# [DONE] copying someone with a gun equpped gives you that gun and you cant take it off
# [DONE] scientist fear noises or text while running
# [DONE] scientists should make out nose when attacked
# [DONE] explosion sound
# [DONE] more guns, rifles and shotguns
# [DONE] difficulty levels
# [DONE] build made of material
# [DONE] new enemeis with shotgun
# [DONE] j and b gun -- baseball bat -- windows and chillds
# [DONE] blair rifle
# [DONE] copper scalpel
# [DONE] childs shotgun
# [DONE] macready jandb
# [DONE] macready should not have the gun delay apply to him flamethrower should not
# [DONE] scientist colours
# [DONE] remove all useless menu options
# [DONE] equip items from map
# [DONE] need windows
# [DONE] increase volume of thing theme
# [DONE] get rid of 'cancel task' character messages
# [DONE] get rid of 8bit explosion sound
# [DONE] clones need to get mad at the other clone
# [DONE] need to add score points for each character
# [DONE] get rid of suspicion killing
# [DONE] music turns off when leaving the lab
# [DONE] toggle tooltips
# [DONE] list of cheats in the cheats menu
# [DONE] macready random colour and the thing random colour
# [DONE] animations count as turns?
# [DONE] macready force throwing arm
# [DONE] right arm should do a bit of damage
# [DONE] unlock no FOV and unlock weapons unlock weapons, and UNLOCK invincibility
# [DONE] turn based
# [DONE] macready wear sombrero
# [DONE] full real time
# [DONE] half score for using cheats
# [DONE] character colour purple
# [DONE] menu sounds
# [DONE] bottle and computer noises
# [DONE] remove main menu story
# [DONE] game victory bonus points
# [DONE] remove object IDs, debug stuff and remove player inventory
# [DONE] add game controls


# More rooms and unique items
# add more dialogue (somebody got to the blood)

# Game release and package materials (jodie)
# credits
# list of easter eggs in manual
...
lab_rooms = ['Dog Cages Room', 'Computer Lab', "MacReady's Room", 'Locker Room', 'Medical Lab', 'Rec Room', 'Generator Room', 'Dining Room', 'Dog Cages Room', 'Dining Room', 'Dog Cages Room']
...
class Message:
	...
	def add_message(self):
		...
		if self.type == 'Action':
			...
			self.colour = libtcod.light_purple
		elif self.type == 'Dialogue':
			self.colour = libtcod.light_green
...
class Object_Memory:
	def __init__(self, original_object):
		...
		self.internal_kinetic_energy = original_object.internal_kinetic_energy
		self.appendage_of = original_object.appendage_of
...
class Object:
	...
	def __init__(self,
		...
		self.last_gun_tick = 0.0
		
		self.seen_the_thing = False
	...
	def play_destroy_sound(self):
		...
		if not HEAR_ALL and not self.name == 'Sniper Rifle Bullet':
			if not libtcod.map_is_in_fov(fov_map, self.x, self.y):
				return False
				
		if self.destroy_sound:
			play_sound_by_name(self.destroy_sound)
			return True
				
		if self.internal_kinetic_energy >= 100:
			if self.material == 'Stone' or self.material == 'Wood':
				super_explosion_break.play()
			elif self.material == 'Flesh':
				super_explosion_flesh.play()
			else:
				explosion_sound.play()
			
		if self.material == 'Glass':
			glass_break_sound.play()
		elif self.material == 'Flesh' or self.material == 'Alien Flesh':
			flesh_destroy_sound.play()
			if self.unit:
				if libtcod.random_get_int(0, 0, 1) == 0:
					if self.name in ['Blair', 'Copper', 'Windows', 'Childs', 'Nauls', 'MacReady', 'Ali Zoheir', 'Jayden Scarpo', 'Oskaar Dahl', 'Mathias Karlsen']:
						male_death.play()
					elif self.name in ['Francis Bubblesmith', 'Ingrid Holm', 'Shira Balewa', 'Jessica Smyth', 'Yollanda Ruud', 'Sophie Strand']:
						female_death.play()
					elif self.name in ['Dog']:
						dog_death.play()
	...
	def toggle_on_off(self, object_to_toggle, force_state = None):
		...
		if object_to_toggle.on_off_state == 'On':
			...
			if object_to_toggle.attached_to_main_power and not main_power_on:
				...
				if object_to_toggle.name == 'Radio':
					PLAY_MUSIC = True
	...
	def establish_properties(self):
		...
		if self.unit:
			if self.unit.unit_name:
				...
				if self.name == 'Ali Zoheir':
					self.foreground_colour = libtcod.green
				elif self.name == 'Jayden Scarpo':
					self.foreground_colour = libtcod.sea
				elif self.name == 'Oskaar Dahl':
					self.foreground_colour = libtcod.turquoise
				elif self.name == 'Mathias Karlsen':
					self.foreground_colour = libtcod.sepia
				elif self.name == 'Francis Bubblesmith':
					self.foreground_colour = libtcod.azure
				elif self.name == 'Ingrid Holm':
					self.foreground_colour = libtcod.amber
				elif self.name == 'Shira Balewa':
					self.foreground_colour = libtcod.copper
				elif self.name == 'Jessica Smyth':
					self.foreground_colour = libtcod.lime
				elif self.name == 'Yollanda Ruud':
					self.foreground_colour = libtcod.fuchsia
				elif self.name == 'Sophie Strand':
					self.foreground_colour = libtcod.yellow
	...
	def can_i_perform_task(self, object, task):
		...
		if task == 'Drag' or task == 'Lift' or task == 'Throw':
			...
			if object.attack_function == 'None':
				return False
	...
	def move(self, dx, dy, force_success = False, dont_repeat = False):
		global map, player, theme_song_started, main_power_on
		...
		if self.unit:
			if self.unit.holding:
				if self.unit.holding in map[self.unit.holding.x][self.unit.holding.y].objects_on_this_tile:
		...
		if not is_blocked(self.x + dx, self.y + dy, self) or (self.x + dx == test_x and self.y + dy == test_y) or force_success:
			...
			if self == player:
				...
				if current_room_name <> 'Outside' and previous_room_name == 'Outside':
					if main_power_on:
						if not PLAY_MUSIC: play_music('new_indoor.ogg')
					else:
						pygame.mixer.music.stop()
					if not theme_song_started and not PLAY_MUSIC:
						theme_song.play()
						theme_song_started = True
				elif current_room_name == 'Outside' and previous_room_name <> 'Outside':
					if not PLAY_MUSIC: play_music('new_outdoor.ogg')
	...
	def throw(self, projectile_object, target = None, keep_going_if_miss = False):
		...
		if projectile_object.affixed_to_ground and not self.name == 'KALO':
			Message(projectile_object.name + ' is affixed to the ground.', 'Narrator', relevant_objects = [self])
			return False
		...
		throw_success = process_projectile(self.unit.throw_weight, projectile_object, specific_target_object_or_tile = target, accuracy_modifier = TARGETED_THROW_ACCURACY_BONUS, pass_through_object = self, attacker_taking_credit = self)
		if target:
	...
	def destroy(self, collision_kinetic_energy = None, cause_of_destroy = None, attacker_taking_credit = None):
		global objects_on_fire, objects_in_motion, player, game_state, scoreboard, player_has_died, dead_characters
		...
		if 'Tentacle' in cause_of_destroy and self.unit:
			player.unit.bonus_weight += int(self.weight / 2)
			...
			if self <> player:
				...
				transform_consume.play()
			
		else:
			if self <> player and self.name <> 'MacReady':
				for inventory_object in self.inventory:
		...
		if self.unit:
			dead_characters.append(self.name)
			
		if self.unit:	
			if self <> player:
				if self.name in ['Nauls', 'MacReady', 'Copper', 'Blair', 'Childs', 'Windows']:
					scoreboard.unique_enemy_killed(self.name, attacker_taking_credit = attacker_taking_credit)
				elif self.unit.creature_type == 'Humanoid':
					scoreboard.humanoid_killed(self.name, attacker_taking_credit = attacker_taking_credit)
				elif self.unit.creature_type == 'Animal':
					scoreboard.dog_killed(self.name, attacker_taking_credit = attacker_taking_credit)
	...
	def check_if_destroyed(self, impact_force = 0, cause_of_destroy = 'None', attacker_taking_credit = None):
		if self.condition <= 0:
			self.destroy(impact_force, cause_of_destroy = cause_of_destroy, attacker_taking_credit = attacker_taking_credit)
...
def play_impact_sound(collided_object, collided_object_damage, moving_object, moving_object_damage):
	global fov_map
	
	if not HEAR_ALL:
		if not libtcod.map_is_in_fov(fov_map, collided_object.x, collided_object.y):
			return False
	
	if collided_object_damage == 0 and moving_object_damage == 0:
		impact_no_damage_sound.play()
		return False
		
	audio_object = None
		
	if moving_object_damage == 0:
		audio_object = collided_object
		
	if collided_object_damage == 0:
		audio_object = moving_object
		
	if get_material_stats_by_name(moving_object.material, 'Weight') < get_material_stats_by_name(collided_object.material, 'Weight'):
		audio_object = moving_object
	else:
		audio_object = collided_object
	...
	if audio_object.material == 'Flesh' or audio_object.material == 'Alien Flesh':
		impact_flesh_sound.play()
		if audio_object.unit:
			if libtcod.random_get_int(0, 0, 2) == 0:
				if audio_object.name in ['Blair', 'Copper', 'Windows', 'Childs', 'Nauls', 'MacReady', 'Ali Zoheir', 'Jayden Scarpo', 'Oskaar Dahl', 'Mathias Karlsen']:
					male_hit.play()
				elif audio_object.name in ['Francis Bubblesmith', 'Ingrid Holm', 'Shira Balewa', 'Jessica Smyth', 'Yollanda Ruud', 'Sophie Strand']:
					female_hit.play()
	elif audio_object.material == 'Plastic':
		impact_plastic_sound.play()
	elif audio_object.material == 'Wood':
		impact_wood_sound.play()
	elif audio_object.material == 'Fire':
		flamethrower_sound.play()
	elif audio_object.material == 'Stone':	
		#stone_hit.play()
		impact_sound.play()
	elif audio_object.material == 'Metal':	
		metal_impact.play()
	elif audio_object.material == 'Cloth':	
		cloth_rip.play()
	else:
		impact_sound.play()
...
class Projectile_In_Motion:
	def __init__(self, projectile_object, specific_target_object_or_tile, velocity, accuracy_modifier, pass_through_object, destination_tile_x, destination_tile_y, secondary_target = None, attacker_taking_credit = None):
		...
		self.turns_taken = 0
		self.attacker_taking_credit = attacker_taking_credit
	...
	def run_turn(self):
		...
		if collision_object:
			if collision_object == self.pass_through_object:
				...
				process_collision(self.projectile_object, collision_object, self.velocity, attacker_taking_credit = self.attacker_taking_credit)
				return True
		...
		if self.projectile_object.name <> 'Sniper Rifle Bullet':
		...
		if self.turns_taken > MAXIMUM_PROJECTILE_DISTANCE and self.projectile_object.name <> 'Sniper Rifle Bullet':
			return True

		return False

def process_projectile(energy_force, projectile_object, specific_target_object_or_tile = None, accuracy_modifier = 0, pass_through_object = None, keep_going_if_miss = False, attacker_taking_credit = None):
	...
	if projectile_object.name == 'Sniper Rifle Bullet':
		accuracy_modifier += STANDARD_ACCURACY_DISTANCE
	...
	this_moving_object = Projectile_In_Motion(projectile_object, specific_target_object_or_tile, velocity, accuracy_modifier, pass_through_object, destination_tile_x, destination_tile_y, secondary_target = secondary_target, attacker_taking_credit = attacker_taking_credit)
...
class Unit:
	...
	def transform(self, transform_into_name, number_of_turns = 3):
		...
		current_condition = self.owner.condition
	
		pick_random([transform_1, transform_2]).play()
		...
		new_player.condition = current_condition
	...
	def sees_the_thing(self, the_thing_object, sees_the_actual_transformation = False):
		...
		previous_feelings = self.check_my_feelings_for(the_thing_object)
			
		new_feelings = self.check_my_feelings_for(the_thing_object, adjust_by = REVEAL_THE_THING_FEELINGS_CHANGE)
		
		if new_feelings < previous_feelings:
			if self.owner.name == 'Dog':
				pick_random([dog_fear_1, dog_fear_2, dog_fear_3]).play()
	...
	def generate_appendages(self):
		...
		generic_appendage.ammunition_for = 'Throwing Arm'
		generic_appendage.attack_function = 'Melee'
		
		if not self.owner.name in ['Nauls', 'Copper', 'MacReady', 'Childs', 'Windows', 'Blair', 'Dog']:
			generic_appendage.internal_kinetic_energy = 500
		...
		if self.owner.the_thing:
			...
			self.rarm.internal_kinetic_energy = 1000
			self.larm.internal_kinetic_energy = 1000
	...
	def determine_task(self, test = False):
		...
		if potential_enemies:
			...
			best_damage_potential = 0
			best_weapon_choice = None
			best_ammo_choice = None

			potential_weapons = []
			
			if self.owner.equipped:
				potential_weapons = potential_weapons + [self.owner.equipped]
		
			weapons_in_inventory = [useful_weapon for useful_weapon in self.owner.inventory if (useful_weapon.attack_function and self.owner.can_i_perform_task(useful_weapon, 'Equip'))]
			
			if weapons_in_inventory:
				potential_weapons = potential_weapons + weapons_in_inventory
			
			nearby_weapons_i_have_seen_before = [useful_weapon for useful_weapon in self.familiar_objects if (useful_weapon.attack_function and not useful_weapon.name in names_of_weapons_i_have and self.owner.can_i_perform_task(useful_weapon, 'Equip') and self.owner.distance_to(useful_weapon) < 10) and self.owner.can_i_perform_task(useful_weapon, 'Lift')]
			
			if nearby_weapons_i_have_seen_before:
				potential_weapons = potential_weapons + nearby_weapons_i_have_seen_before
				
			for potential_weapon in potential_weapons:
				
				chosen_ammo = None
				
				if potential_weapon.attack_function <> 'Melee':
					i_have_useful_ammo = [useful_ammo_2 for useful_ammo_2 in self.owner.inventory if potential_weapon.name in useful_ammo_2.ammunition_for]
					
					chosen_ammo = pick_random(i_have_useful_ammo)
					
					if not chosen_ammo:
					
						nearby_ammo_ive_seen_before = [ammo_ive_seen for ammo_ive_seen in self.familiar_objects if (potential_weapon.name in ammo_ive_seen.ammunition_for and not ammo_ive_seen.name in names_of_ammo_i_have and self.owner.distance_to(ammo_ive_seen) < 10) and self.owner.can_i_perform_task(useful_weapon, 'Lift')]
						
						chosen_ammo = pick_random(nearby_ammo_ive_seen_before)
				...
				if chosen_ammo:
					total_power = calculate_velocity(potential_weapon.internal_kinetic_energy + chosen_ammo.internal_kinetic_energy, chosen_ammo.weight)
				else:
					total_power = calculate_velocity(self.throw_weight + potential_weapon.internal_kinetic_energy, potential_weapon.weight)
					
				if isinstance(potential_weapon, Object_Memory):
					total_power = int(total_power / 2)
				
				if chosen_ammo:
					if isinstance(chosen_ammo, Object_Memory):
						total_power = int(total_power / 2)
						
				if potential_weapon.name == 'Throwing Arm':
					total_power = total_power * 10
					
				if total_power:
					if total_power > best_damage_potential:
						best_damage_potential = total_power
						best_weapon_choice = potential_weapon
						best_ammo_choice = chosen_ammo
				
				if self.owner.name == 'MacReady':
					best_weapon_choice = None
					best_ammo_choice = None
					
					if 'Throwing Arm' in names_of_weapons_i_have:
						for mac_weapon in self.owner.inventory:
							if mac_weapon.name == 'Throwing Arm':
								best_weapon_choice = mac_weapon
								break
								
					if best_weapon_choice:
						if 'J&B' in names_of_ammo_i_have:
							for mac_ammo in self.owner.inventory:
								if mac_ammo.name == 'J&B':
									best_ammo_choice = mac_ammo
									break
						
			if best_weapon_choice:
				if self.owner.equipped == best_weapon_choice:
					pass
				elif best_weapon_choice in self.owner.inventory:
					self.equip(best_weapon_choice)
				else:
					chosen_task = 'Find and Collect Object'
					priority = 8
					chosen_object_or_room = best_weapon_choice
				
				if best_weapon_choice.attack_function <> 'Melee':
					if best_ammo_choice:
						if best_ammo_choice in self.owner.inventory:
							pass
						else:
							chosen_task = 'Find and Collect Object'
							priority = 8
							chosen_object_or_room = best_ammo_choice
							quantity = 10
				
				if best_weapon_choice == 'Throwing Arm':
					i_have_useful_ammo = [useful_ammo_2 for useful_ammo_2 in self.owner.inventory if potential_weapon.name in useful_ammo_2.ammunition_for]
					if not i_have_useful_ammo:
	...
	def attack(self, target_object_or_tile = None, test = False):
		...
		ammunition = None
		if self.owner.equipped.attack_function <> 'Melee' and self.owner.equipped.attack_function <> 'None':
			i_have_useful_ammo = [useful_ammo_2 for useful_ammo_2 in self.owner.inventory if self.owner.equipped.name in useful_ammo_2.ammunition_for]
			if i_have_useful_ammo:
				ammunition = pick_random(i_have_useful_ammo)
		...
		if test and isinstance(target_object_or_tile, Object):
			...
			if not ammunition:
				...
				velocity = calculate_velocity(self.owner.equipped.internal_kinetic_energy + ammunition.internal_kinetic_energy, ammunition.weight)
				test_damage = process_collision(ammunition, target_object_or_tile, velocity, test = test)

			return test_damage
			
		if self.owner.equipped:
			if self.owner.equipped.name == 'Throwing Arm' and not ammunition:
				return False
			
		if self.owner.distance_to(target_object_or_tile) > self.owner.equipped.attack_range and self.owner.equipped.attack_function == 'Melee':
		...
		if isinstance(target_object_or_tile, Object):
			if self.owner.equipped:
				if ammunition:
					Message(self.owner.name + ' attacks ' + target_object_or_tile.name + ' with ' + ammunition.name + ' using ' + self.owner.equipped.name + '.', 'Action', relevant_objects=[self.owner, target_object_or_tile])
				else:
					Message(self.owner.name + ' attacks ' + target_object_or_tile.name + ' with ' + self.owner.equipped.name + '.', 'Action', relevant_objects=[self.owner, target_object_or_tile])
			else:
				Message(self.owner.name + ' attacks ' + target_object_or_tile.name + '.', 'Action', relevant_objects=[self.owner, target_object_or_tile])
		
		if HEAR_ALL or libtcod.map_is_in_fov(fov_map, self.owner.x, self.owner.y):
			if self.owner.name == 'Dog':
				if libtcod.random_get_int(0, 0, 3) == 0:
					dog_attack.play()
			
			if 'Right Arm' in self.owner.equipped.name or 'Left Arm' in self.owner.equipped.name:
				pick_random([new_hit_1, new_hit_2]).play()
			elif 'Leg' in self.owner.equipped.name:
				new_kick_1.play()
			elif 'Tentacle' in self.owner.equipped.name:
				pick_random([new_tentacle_hit_1, new_tentacle_hit_2]).play()
			elif 'Throwing Arm' in self.owner.equipped.name:
				if libtcod.random_get_int(0, 0, 1) == 0:
					throw_whoosh.play()

		if self.owner.equipped.attack_function:
			if self.owner.equipped.attack_function == 'Melee':
				mute_impact_sound = False
				if self.owner.equipped.attack_sound:
					play_sound_by_name(self.owner.equipped.attack_sound)
					if libtcod.random_get_int(0, 0, 2) == 0:
						mute_impact_sound = True
				...
				process_collision(self.owner.equipped, target_object_or_tile, velocity, mute_impact_sound = mute_impact_sound, attacker_taking_credit = self.owner)
			...
			elif self.owner.equipped.attack_function <> 'None':
				if not ammunition:
					Message('No ammo', 'Action', relevant_objects=[self.owner])
					no_ammo_sound.play()
					return False
				else:
					if libtcod.sys_elapsed_seconds() > self.owner.last_gun_tick + gun_timer or self.owner == player:
						self.owner.last_gun_tick = libtcod.sys_elapsed_seconds()
						dropped_ammunition = self.owner.drop(ammunition, 1)
						shotgun_blast = [dropped_ammunition]
						if self.owner.equipped.name == 'Shotgun':
							shotgun_blast.append(copy.deepcopy(dropped_ammunition))
							shotgun_blast.append(copy.deepcopy(dropped_ammunition))
							shotgun_blast.append(copy.deepcopy(dropped_ammunition))
							shotgun_blast.append(copy.deepcopy(dropped_ammunition))
							for ammo in shotgun_blast:
								amount_of_sway = int(self.owner.distance_to(target_object_or_tile) / 2)
								target = map[normalize_to_map_width(target_object_or_tile.x + libtcod.random_get_int(0, 0 - amount_of_sway, amount_of_sway))][normalize_to_map_height(target_object_or_tile.y + libtcod.random_get_int(0, 0 - amount_of_sway, amount_of_sway))]
								process_projectile(self.owner.equipped.internal_kinetic_energy, ammo, target, accuracy_modifier = 5, keep_going_if_miss = True, attacker_taking_credit = self.owner)
						else:
							process_projectile(self.owner.equipped.internal_kinetic_energy, dropped_ammunition, target_object_or_tile, accuracy_modifier = 5, keep_going_if_miss = True, attacker_taking_credit = self.owner )
						...
						if self.owner.equipped.name == 'Flamethrower':
							...
							if self.owner.equipped.name == 'Gun':
								gun_sound.play()
							elif self.owner.equipped.name == 'Sniper Rifle':
								rifle_sound.play()
							elif self.owner.equipped.name == 'Shotgun':
								shotgun_sound.play()
		elif isinstance(target_object_or_tile, Object):
			velocity = calculate_velocity(self.throw_weight, self.owner.equipped.weight)
			previous_state = self.owner.equipped.indestructible
			self.owner.equipped.indestructible = 0.2
			process_collision(self.owner.equipped, target_object_or_tile, velocity, attacker_taking_credit = self.owner)
			if target_object_or_tile.unit:
				self.intentional_attack(target_object_or_tile)
			if self.owner.equipped:
				self.owner.equipped.indestructible = previous_state
		...
		if isinstance(target_object_or_tile, Object):
			if target_object_or_tile.unit:
				for visible_unit in units:
					if visible_unit == self.owner:
						continue
					if visible_unit == player:
						continue
					if visible_unit.distance_to(self.owner) < 5:
						if visible_unit.unit.check_my_feelings_for(target_object_or_tile) > 0:
							if visible_unit.unit.check_my_feelings_for(self.owner) > 0:
								visible_unit.unit.check_my_feelings_for(self.owner, adjust_by = -3)
								visible_unit.unit.speak_to(self.owner, "The hell you doing attacking " + target_object_or_tile.name + ".")
								if visible_unit.unit.check_my_feelings_for(self.owner) <= 0:
									visible_unit.unit.speak_to(self.owner, "That's it, you gotta die!")
	...
	def speak_to(self, spoken_to_object, given_dialogue = None, given_dialogue_ID = None):
		...
		if self.owner.name == 'Dog':
			Message(self.owner.name + ' to ' + spoken_to_object.name + ': ' + 'Bark bark!', 'Dialogue', relevant_objects=[self.owner, spoken_to_object])
			return False
	...
	def scan_visible_area(self, default_range = int(MAX_VISIBILITY_RADIUS)):
		global fov_map, dead_characters, player
			...
			for y in range(start_y, end_y):
				...
				for observed_object in map[x][y].objects_on_this_tile:
					...
					if observed_object.unit:					
						...
						# The Thing
						if (observed_object.the_thing and observed_object == player) or observed_object.name == 'KALO':
							if not self.owner.seen_the_thing:
								Message(self.owner.name + ' to ' + observed_object.name + ": It's The Thing! Kill it!", "Dialogue")
								self.owner.seen_the_thing = True
							self.sees_the_thing(observed_object)
							
						# The Thing
						if observed_object.id == self.owner.id and observed_object == player:
							if not self.owner.seen_the_thing:
								Message(self.owner.name + ' to ' + observed_object.name + ": It's The Thing! Kill it!", "Dialogue")
								self.owner.seen_the_thing = True
							self.sees_the_thing(observed_object)
						...
						# The Thing
						if observed_object.name == self.owner.name and observed_object == player:
							if not self.owner.seen_the_thing:
								Message(self.owner.name + ' to ' + observed_object.name + ": oh my god are you pretending to be me!?  What the fuck?", "Dialogue")
								self.owner.seen_the_thing = True

							self.sees_the_thing(observed_object)
							
						if observed_object.name in dead_characters and observed_object == player:
							if not self.owner.seen_the_thing:
								Message(self.owner.name + ' to ' + observed_object.name + ": ...wait, I thought you were dead?  Shit you're The Thing!", "Dialogue")
								self.owner.seen_the_thing = True
	...
	def pick_up(self, object_to_pick_up, quantity = None):
		...
		# Removed the inventory carry restriction
		"""
...
def calculate_velocity(throw_weight, weight):
	...
	velocity = int(throw_weight / (weight * 2))
...
def process_collision(moving_object, collided_object, velocity, test = False, mute_impact_sound = False, attacker_taking_credit = None):
	...
	if libtcod.map_is_in_fov(fov_map, collided_object.x, collided_object.y) or (moving_object.name == 'Sniper Rifle Bullet'):
		if not mute_impact_sound: play_impact_sound(collided_object, collided_object_damage, moving_object, moving_object_damage)
		
		if moving_object.unit or collided_object.unit:
	...
	collided_object.check_if_destroyed(impact_force = collision_kinetic_energy, cause_of_destroy = moving_object.name, attacker_taking_credit = attacker_taking_credit)
...
def get_adjacent_stuff(x, y, what_im_looking_for, include_diagonals=False, include_centre=False):
	...
	centre = map[normalize_to_map_width(x)][normalize_to_map_height(y)]
	right = map[normalize_to_map_width(x + 1)][normalize_to_map_height(y)]
	left = map[normalize_to_map_width(x - 1)][normalize_to_map_height(y)]
	up = map[normalize_to_map_width(x)][normalize_to_map_height(y - 1)]
	down = map[normalize_to_map_width(x)][normalize_to_map_height(y + 1)]
	upright = map[normalize_to_map_width(x + 1)][normalize_to_map_height(y - 1)]
	upleft = map[normalize_to_map_width(x - 1)][normalize_to_map_height(y - 1)]
	downright = map[normalize_to_map_width(x + 1)][normalize_to_map_height(y + 1)]
	downleft = map[normalize_to_map_width(x - 1)][normalize_to_map_height(y + 1)]
...
def find_random_tile_for_object(object, room_name_exclude = None):
	global map
	object_placed = False
	...
	while not object_placed:
		...
		if not is_blocked(x, y, check_against_object = object):
			...
			if object.size + test_wall.size >= 10:
				...
				if room_name_exclude:
					if map[x][y].room_name == room_name_exclude:
						object_placed = False
	...
	for stuff_to_place in room_type_object.objects_in_room:
		...
		for i in range(0, object_or_group_quantity):
			...
			if object_or_group == 'Group':
				...
				we_have_duplicates = True
				while(we_have_duplicates):
					we_have_duplicates = False
					placed_object = get_object_by_name(object_or_group_name)
					if placed_object.unit:
						if placed_object.unit.creature_type == 'Humanoid':
							for check_unit in units:
								if placed_object.name == check_unit.name:
									we_have_duplicates = True
									break
...
def handle_keyboard():
	global key, player, units, global_render_tooltips
	...
	if key_char == 'w' or key_char == 'W':
		...
		if not move_success:
			return player.unit.try_to_manoeuvre(1, 0)
	elif key_char == 'f' or key_char == 'F' or key_char == 'r' or key_char == 'R' or key_char == 'e' or key_char == 'E' or key_char == 'c' or key_char == 'C' or key_char == 'v' or key_char == 'V' or key_char == 'z' or key_char == 'Z':
		...
		if adjacent_objects:
			...
			if selected_object:
				...
				if key_char == 'f' or key_char == 'F':
					...
					player.unit.speak_to(selected_object)
					return selected_object.unit.speak_to(player)
		...
		if player.inventory:
			...
			if chosen_inventory_object:
				...
				action_options = ['Equip', 'Drop', 'Throw']
	...
	elif key_char == 'u' or key_char == 'U':
		names = ['#Puny Life Forms Remaining: ' + str(len(units) - 1), '']
		for unit in units:
			if unit <> player:
				names.append(unit.name)
		chosen_action = menu_prompt(written_text = names, header = 'Units Remaining')
		
	elif key_char == 'k' or key_char == 'K':
		controls_menu_text = []
		controls_menu_text.append('R.Clk - Attack Object')
		controls_menu_text.append('L.Clk - Display Object Info in Panel')
		controls_menu_text.append('    K - Controls Menu')
		controls_menu_text.append('    E - Grab and Drag Object')
		controls_menu_text.append('    Q - Inventory')
		controls_menu_text.append('    Y - Transformation Menu')
		controls_menu_text.append('    U - Units Remaining')
		controls_menu_text.append('    T - Wait One Turn (Transform)')
		controls_menu_text.append('    R - Open / Close Door')
		controls_menu_text.append('    C - Lock / Unlock Object')
		controls_menu_text.append('    F - Pick Up Object')
		controls_menu_text.append('    V - Turn Object On/Off')
		controls_menu_text.append('    Z - Speak to Object')
		controls_menu_text.append('    X - Toggle Lock Player Movement')
		controls_menu_text.append('    P - Toggle Mouseover Panels')
		chosen_action = menu_prompt(written_text = controls_menu_text, header = 'Controls')
	elif key_char == 'p' or key_char == 'P':
		if global_render_tooltips:
			global_render_tooltips = False
			Message('Tooltip rendering Off.', 'Debug')
		else:
			global_render_tooltips = True
			Message('Tooltip rendering On.', 'Debug')
...
def toggle_main_power(force_state = None):
	...
	if main_power_on == True:
		...
		if map[player.x][player.y].room_name <> 'Outside':
			pygame.mixer.music.stop()
	else:
		main_power_on = True
...
def new_game():
	...
	global SCALPEL_ACTIVE
	global GUN_ACTIVE
	global FLAMETHROWER_ACTIVE
	global ROCKET_ACTIVE
	global SHOTGUN_ACTIVE
	global MEGA_BOMB_ACTIVE
	global UNLIMITED_AMMO_ACTIVE
	global RIFLE_ACTIVE
	...
	player.description = "An alien creature with origins unknown."
	...
	if not player.name:
		player.name = pick_random(['Sleve McDichael', 'Rey McScriff', 'Willie Dustice', 'Todd Bonzalez', 'Bobson Dugnutt', 'Tim Sandaele', 'Dean Wesrey', 'Tony Smehrik', 'Kevin Nogilny', 'Scott Dourque', 'Anatoli Smorin', 'Mike Truk', 'Karl Dandleton', 'Onson Sweemey', 'Glenallen Mixon', 'Raul Chamgerlain'])
	
	player.name = player.name.replace("\r","")
	...
	if SCALPEL_ACTIVE:
		bonus_weapon = get_object_by_name('Scalpel')
		player.add_to_inventory(bonus_weapon)

	if GUN_ACTIVE:
		bonus_weapon = get_object_by_name('Gun')
		bonus_ammo = get_object_by_name('Bullet')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_weapon)
		player.add_to_inventory(bonus_ammo)

	if FLAMETHROWER_ACTIVE:
		bonus_weapon = get_object_by_name('Flamethrower')
		bonus_ammo = get_object_by_name('Flame Tank')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_weapon)
		player.add_to_inventory(bonus_ammo)

	if ROCKET_ACTIVE:
		bonus_weapon = get_object_by_name('Rocket Launcher')
		bonus_ammo = get_object_by_name('Rocket')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_weapon)
		player.add_to_inventory(bonus_ammo)

	if SHOTGUN_ACTIVE:
		bonus_weapon = get_object_by_name('Shotgun')
		bonus_ammo = get_object_by_name('Shotgun Shell')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_weapon)
		player.add_to_inventory(bonus_ammo)
		
	if RIFLE_ACTIVE:
		bonus_weapon = get_object_by_name('Sniper Rifle')
		bonus_ammo = get_object_by_name('Sniper Rifle Bullet')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_weapon)
		player.add_to_inventory(bonus_ammo)
		
	if MEGA_BOMB_ACTIVE:
		bonus_ammo = get_object_by_name('Mega Bomb')
		bonus_ammo.quantity = 10
		player.add_to_inventory(bonus_ammo)
	...
	if INVINCIBLE_ACTIVE:
		player.material = 'Titanium'
		player.update_weight()
		player.unit.update_strength(100)
	...
	for i in range (0, 8):
		...
		if exterior_door_chosen_spot:
			...
			exterior_door_object.previous_foreground_colour = libtcod.light_cyan
	...
	for unit in units:
		copy_names = [object.name for object in player.unit.transform_options if object.name == unit.name]
		
		if not copy_names:
			new_transform = copy.deepcopy(unit)
			remove_all_connections_to_object(new_transform)
			player.unit.transform_options.append(new_transform)
...
def play_game():
	global key, mouse, game_paused, clicked_object_name, fov_recompute, game_state, all_dialogue_IDs, player, objects_on_fire, objects_in_motion, object_info_object, fov_map, player_has_died, gun_timer, difficulty_level, scoreboard, main_power_on, PLAY_MUSIC, cheats_on
	...
	gun_timer = DEFAULT_GUN_TIMER
		
	if difficulty_level in ['Normal', 'Hard', 'Very Hard', 'Insane']:

		object_to_place = get_object_by_name('Nauls')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.unit.dialogue.append(327)
		
		if difficulty_level in ['Very Hard', 'Insane']:
			object_to_place.indestructible = 0.75
		...
		object_to_place.unit.dialogue.append(600)
		object_to_place.unit.dialogue.append(601)
		object_to_place.unit.dialogue.append(602)
		object_to_place.unit.dialogue.append(603)
		object_to_place.unit.dialogue.append(604)
		object_to_place.unit.dialogue.append(605)
		object_to_place.unit.dialogue.append(606)
		
		if difficulty_level in ['Very Hard', 'Insane']:
			object_to_place.indestructible = 0.5

		
		for item in object_to_place.inventory:
			if item.name == 'Throwing Arm':
				item.internal_kinetic_energy = 100000
			elif item.name == 'J&B':
				item.internal_kinetic_energy = 100000
		
	if difficulty_level in ['Hard', 'Very Hard', 'Insane']:
		object_to_place = get_object_by_name('Copper')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.name = 'Copper'
		
		object_to_place.unit.dialogue.append(615)
		object_to_place.unit.dialogue.append(616)
		
		if difficulty_level in ['Very Hard', 'Insane']:
			object_to_place.indestructible = 0.75
		
		object_to_place = get_object_by_name('Blair')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.name = 'Blair'
		
		object_to_place.unit.dialogue.append(610)
		object_to_place.unit.dialogue.append(611)
		object_to_place.unit.dialogue.append(612)
		object_to_place.unit.dialogue.append(613)
	...
	if difficulty_level in ['Very Hard', 'Insane']:
		...
		object_to_place = get_object_by_name('Childs')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.name = 'Childs'
		object_to_place.unit.dialogue.append(606)
		object_to_place.unit.dialogue.append(607)
		object_to_place.unit.dialogue.append(608)
		object_to_place.unit.dialogue.append(609)
		
		if difficulty_level in ['Insane']:
			object_to_place.indestructible = 0.5
		
		object_to_place = get_object_by_name('Windows')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.name = 'Windows'
		object_to_place.unit.dialogue.append(617)
		object_to_place.unit.dialogue.append(618)
		object_to_place.unit.dialogue.append(619)
		
		if difficulty_level in ['Insane']:
			object_to_place.indestructible = 0.5
		
	a = (player.name.lower().strip())
	d = (('chicken bike').lower().strip())
	
	kalo = None
	
	if a == d:
		
		object_to_place = get_object_by_name('KALO')
		place_tile = find_random_tile_for_object(object_to_place, room_name_exclude = 'Outside')
		place_object_at_location(object_to_place, place_tile.x, place_tile.y)
		object_to_place.name = 'KALO'
		player.unit.update_strength(1000)
		cheats_on = True
		for item in object_to_place.inventory:
			if item.name == 'Throwing Arm':
				item.internal_kinetic_energy = 100000
		kalo = object_to_place
	else:
		if not SCALPEL_ACTIVE and not GUN_ACTIVE and not FLAMETHROWER_ACTIVE and not ROCKET_ACTIVE and not SHOTGUN_ACTIVE and not MEGA_BOMB_ACTIVE and not UNLIMITED_AMMO_ACTIVE and not RIFLE_ACTIVE and not INVINCIBLE_ACTIVE:
			cheats_on = False
	...
	a = (player.name.lower().strip())
	b = (('Casey Ryback').lower().strip())
	c = (('William Strannix').lower().strip())
	...
	if kalo:
		for unit in units:
			kalo.unit.check_my_feelings_for(unit, -10)
			
			
	if a == c:
		menu_prompt(written_text = ['+All cheats unlocked!'])
		scoreboard.scalpel_locked = False
		scoreboard.gun_locked = False
		scoreboard.rifle_locked = False
		scoreboard.real_time_locked = False
		scoreboard.skin_change_locked = False
		scoreboard.flamethrower_locked = False
		scoreboard.rocket_locked = False
		scoreboard.shotgun_locked = False
		scoreboard.fov_locked = False
		scoreboard.mega_bomb_locked = False
		scoreboard.ammo_locked = False
		scoreboard.invincible_locked = False
		scoreboard.lab_material_change_locked = False
	...
	while not libtcod.console_is_window_closed() and game_state == 'playing' and (not player_has_died or len(objects_in_motion) > 0):
		...
		if (player.unit.turn_taken == True and libtcod.sys_elapsed_seconds() > last_tick + NUMBER_OF_SECONDS_BETWEEN_TURNS) or (REAL_TIME_ACTIVE and libtcod.sys_elapsed_seconds() > last_tick + NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME):

			if (REAL_TIME_ACTIVE and libtcod.sys_elapsed_seconds() > last_tick + NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME) or not REAL_TIME_ACTIVE:
				last_tick = libtcod.sys_elapsed_seconds()
		...
		if len(units) <= 1:
			...
			play_music('bto.ogg')
			menu_prompt(written_text = ['#Congratulations! ' + player.name + ' has eliminated all life forms at Outpost 31.', '', 'You are a shining example of your disgusting species.', ''])
			scoreboard.session_victory = True
			end_game_return_to_main_menu()
		
	if player_has_died:
		theme_song.stop()
		pick_random([thing_dies_1, thing_dies_2]).play()
		if player.condition <= 0:
			this = pick_random(['louis_1.ogg', 'louis_2.ogg', 'louis_3.ogg', 'louis_4.ogg'])
			pygame.mixer.music.set_volume(0.4)
			play_music(this)
		menu_prompt(written_text = ['#' + player.name + ' has been killed.', '', 'The inhabitants of Outpost 31 will now continue their research in peace.', '', 'You have brought shame to your weird alien family.', ''])
		end_game_return_to_main_menu()
...
def initialize_sounds():
	...
	global new_tentacle_hit_1
	global new_tentacle_hit_2
	
	global transform_1
	global transform_2
	global transform_consume
	...
	global super_explosion_break
	global super_explosion_flesh
	
	global stone_hit
	global metal_impact
	
	global rifle_sound
	global shotgun_sound
	...
	global bottle_break
	global bottle_break_2
	global bullet_hit
	
	global computer_noise
	global computer_noise_2
	global computer_noise_3
	global computer_noise_4
	...
	explosion_sound = pygame.mixer.Sound('thing_sounds/explosion.ogg')
	...
	dog_attack = pygame.mixer.Sound('thing_sounds/new_dog_attack.ogg')
	
	new_kick_1 = pygame.mixer.Sound('thing_sounds/new_kick_1.ogg')
	new_hit_1 = pygame.mixer.Sound('thing_sounds/new_hit_1.ogg')
	new_hit_2 = pygame.mixer.Sound('thing_sounds/new_hit_2.ogg')
	new_hit_4 = pygame.mixer.Sound('thing_sounds/new_hit_4.ogg')
	
	new_tentacle_hit_1 = pygame.mixer.Sound('thing_sounds/new_tentacle_hit_1.ogg')
	new_tentacle_hit_2 = pygame.mixer.Sound('thing_sounds/new_tentacle_hit_2.ogg')

	transform_1 = pygame.mixer.Sound('thing_sounds/new_transform_1.ogg')
	transform_2 = pygame.mixer.Sound('thing_sounds/new_transform_2.ogg')
	transform_consume = pygame.mixer.Sound('thing_sounds/new_consume.ogg')
	
	transform_consume = pygame.mixer.Sound('thing_sounds/new_consume.ogg')
	
	dog_fear_1 = pygame.mixer.Sound('thing_sounds/new_dog_fear_1.ogg')
	dog_fear_2 = pygame.mixer.Sound('thing_sounds/new_dog_fear_2.ogg')
	dog_fear_3 = pygame.mixer.Sound('thing_sounds/new_dog_fear_3.ogg')

	thing_dies_1 = pygame.mixer.Sound('thing_sounds/thing_dies_1.ogg')
	thing_dies_2 = pygame.mixer.Sound('thing_sounds/thing_dies_2.ogg')
	
	wood_break = pygame.mixer.Sound('thing_sounds/wood_break.ogg')

	male_hit = pygame.mixer.Sound('thing_sounds/male_hit.ogg')
	female_hit = pygame.mixer.Sound('thing_sounds/female_hit.ogg')
	
	male_death = pygame.mixer.Sound('thing_sounds/male_death.ogg')
	female_death = pygame.mixer.Sound('thing_sounds/female_death.ogg')
	
	super_explosion_break = pygame.mixer.Sound('thing_sounds/super_explosion_break.ogg')
	super_explosion_flesh = pygame.mixer.Sound('thing_sounds/super_explosion_flesh.ogg')
	
	stone_hit = pygame.mixer.Sound('thing_sounds/stone_hit.ogg')
	metal_impact = pygame.mixer.Sound('thing_sounds/metal_impact.ogg')
	
	rifle_sound = pygame.mixer.Sound('thing_sounds/rifle_sound.ogg')
	shotgun_sound = pygame.mixer.Sound('thing_sounds/shotgun_sound.ogg')
	
	cloth_rip = pygame.mixer.Sound('thing_sounds/cloth_impact.ogg')
	
	axe_hit = pygame.mixer.Sound('thing_sounds/axe_hit.ogg')
	knife_hit = pygame.mixer.Sound('thing_sounds/knife.ogg')
	
	bottle_break = pygame.mixer.Sound('thing_sounds/bottle_break_1.ogg')
	bottle_break_2 = pygame.mixer.Sound('thing_sounds/bottle_break_2.ogg')
	
	bullet_hit = pygame.mixer.Sound('thing_sounds/bullet_hit.ogg')
	
	dog_death = pygame.mixer.Sound('thing_sounds/dog_death.ogg')
	
	computer_noise = pygame.mixer.Sound('thing_sounds/computer_noise.ogg')
	computer_noise_2 = pygame.mixer.Sound('thing_sounds/computer_noise_2.ogg')
	computer_noise_3 = pygame.mixer.Sound('thing_sounds/computer_noise_3.ogg')
	computer_noise_4 = pygame.mixer.Sound('thing_sounds/computer_noise_4.ogg')
	
	menu_noise = pygame.mixer.Sound('thing_sounds/menu_noise.ogg')
	menu_noise_2 = pygame.mixer.Sound('thing_sounds/menu_noise_2.ogg')
	
	menu_click = pygame.mixer.Sound('thing_sounds/menu_click.ogg')
	open_menu = pygame.mixer.Sound('thing_sounds/open_menu.ogg')
	
	throw_whoosh = pygame.mixer.Sound('thing_sounds/throw_whoosh.ogg')
	
	louis_1 = pygame.mixer.Sound('thing_sounds/louis_1.ogg')
	louis_2 = pygame.mixer.Sound('thing_sounds/louis_2.ogg')
	louis_3 = pygame.mixer.Sound('thing_sounds/louis_3.ogg')
	louis_4 = pygame.mixer.Sound('thing_sounds/louis_4.ogg')
...
def end_game_return_to_main_menu():
	global game_state, scoreboard, cheats_on
	game_state = None
	render_all()
	scoreboard.calculate_bonuses()
	written_text = scoreboard.return_session_scores_as_text_array()
	menu_prompt(written_text = written_text)
	pygame.mixer.music.set_volume(1)
	play_music(None)
	if cheats_on:
		menu_prompt(written_text = "Cheats enabled.  Score will not be recorded.", header = 'Cheats On', render_screen_before_displaying = False, main_screen_console = True)
	else:
		save_scoreboard()
		scoreboard.check_for_unlockables()
		save_scoreboard()
	return True
...
def play_sound_by_name(sound_name):
	if sound_name == 'knife_hit':
		knife_hit.play()
	elif sound_name == 'axe_hit':
		axe_hit.play()
	elif sound_name == 'bottle_break':
		pick_random([bottle_break, bottle_break_2]).play()
	elif sound_name == 'bullet_hit':
		bullet_hit.play()
	elif sound_name == 'computer_noise':
		pick_random([computer_noise, computer_noise_2, computer_noise_3, computer_noise_4]).play()
...
def get_room_type_by_name(room_type_name):
	...
	if room_type_name == 'Dog Cages Room':
		...
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Shotgun',
			'minimum_quantity': 2,
			'maximum_quantity': 4})
			
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Sniper Rifle',
			'minimum_quantity': 2,
			'maximum_quantity': 4})
		...
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Shotgun Shell',
			'minimum_quantity': 2,
			'maximum_quantity': 6})
			
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Sniper Rifle Bullet',
			'minimum_quantity': 2,
			'maximum_quantity': 6})
		...
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Locker',
			'minimum_quantity': 1,
			'maximum_quantity': 3})
		...
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Trash Can',
			'minimum_quantity': 1,
			'maximum_quantity': 3})
			
		this_room.objects_in_room.append({
			'object_or_group': 'Group',
			'name_of_object_or_group': 'Table and Chairs',
			'minimum_quantity': 0,
			'maximum_quantity': 1})
	...
	elif room_type_name == 'Dining Room':
	
		this_room = Room_Type("Dining Room")
		...
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Kitchen Knife',
			'minimum_quantity': 1,
			'maximum_quantity': 3})
			
		this_room.objects_in_room.append({
			'object_or_group': 'Object',
			'name_of_object_or_group': 'Cutting Board',
			'minimum_quantity': 1,
			'maximum_quantity': 2})
...
def get_object_group_by_name(object_group_name):
	...
	if object_group_name == 'Table and Chairs':
		# Repeat the name again
		this_object_group = Object_Group(object_group_name, grid=[
		...
		this_object_group.object_map.append(['T', 'Table'])
		this_object_group.object_map.append(['c', 'Chair'])

		this_object_group.place_against_wall = None
...
def get_object_by_name(object_name):
	...
	# Materials
		...
		this_object.ammunition_for = ['Throwing Arm']
	...
	# End materials
		...
		this_object.size = 3
		this_object.material = 'Glass'
		this_object.components = ['Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel', 'Shrapnel']
		this_object.foreground_colour = libtcod.purple
		this_object.description = "Don't fuck with this big ass bomb"
		this_object.internal_kinetic_energy = 10000000
		...
		this_object.on_function = play_sound_by_name
		this_object.on_argument = 'computer_noise'
		
		this_object.off_function = play_sound_by_name
		this_object.off_argument = ''
		...
		this_object.destroy_sound = 'bottle_break'
		...
		this_object.foreground_colour = libtcod.light_grey
		this_object.attack_function = 'Gun'
		this_object.attack_range = 30
		this_object.internal_kinetic_energy = 2000000
		
	elif object_name == 'Shotgun':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 'Z'
		this_object.size = 1
		this_object.material = 'Metal'
		...
		this_object.foreground_colour = libtcod.dark_red
		this_object.attack_function = 'Melee'
		this_object.attack_range = 1
		this_object.internal_kinetic_energy = 500
		this_object.attack_sound = 'axe_hit'
		this_object.attack_function = 'Melee'
		
	elif object_name == 'Oversized Kalo Arm':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 'A'
		this_object.size = 3
		this_object.material = 'Metal'
		...
		this_object.attack_function = 'Melee'
		this_object.attack_range = 1
		this_object.internal_kinetic_energy = 50000
		this_object.attack_sound = 'axe_hit'
		this_object.attack_function = 'Melee'
		

		
	elif object_name == 'Throwing Arm':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 'a'
		this_object.size = 1
		this_object.material = 'Flesh'
		...
		this_object.internal_kinetic_energy = 1500
		...
		this_object.destroy_sound = 'bullet_hit'
		
	elif object_name == 'Sniper Rifle Bullet':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 'q'
		this_object.size = 0.1
		this_object.stackable = True
		...
		this_object.material = 'Metal'
		this_object.foreground_colour = libtcod.light_grey
		this_object.ammunition_for = ['Sniper Rifle']
		this_object.destroy_sound = 'bullet_hit'
		
	elif object_name == 'Shotgun Shell':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 's'
		this_object.size = 0.1
		this_object.stackable = True
		...
		this_object.material = 'Metal'
		this_object.foreground_colour = libtcod.light_grey
		this_object.ammunition_for = ['Shotgun']
		this_object.destroy_sound = 'bullet_hit'
		...
		this_object.attack_sound = 'knife_hit'
		this_object.internal_kinetic_energy = 250
		this_object.attack_function = 'Melee'
		
	elif object_name == 'Kitchen Knife':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = '!'
		this_object.size = 1
		this_object.material = 'Metal'
	...
	elif object_name == 'Cutting Board':
		this_object = get_predesigned_object_by_name('Generic Object')
		this_object.char = 'b'
		this_object.size = 1
		this_object.material = 'Wood'
		this_object.foreground_colour = libtcod.dark_orange
		this_object.attack_sound = 'knife_hit'
		this_object.internal_kinetic_energy = 100
		this_object.attack_function = 'Melee'
	...
	# UNITS BELOW THIS LINE
		...
		this_object.unit = get_unit_by_name('Player')
		this_object.light_source = 'Omni'
		...
		this_object.inventory = ['Throwing Arm', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B', 'J&B']
		this_object.unit = get_unit_by_name('MacReady')
		this_object.material = 'Metal'
		
	elif object_name == 'Windows':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.dark_orange
		this_object.description = "Windows"
		this_object.inventory = ['Flamethrower', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank', 'Flame Tank']
		this_object.unit = get_unit_by_name('Windows')
	
		
	elif object_name == 'KALO':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.light_fuchsia
		this_object.description = "That's KALO."
		this_object.material = "Titanium"
		this_object.inventory = ['Throwing Arm', 'Oversized Kalo Arm']
		this_object.unit = get_unit_by_name('KALO')	

		
	elif object_name == 'Childs':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.dark_yellow
		this_object.description = "Childs"
		this_object.inventory = ['Shotgun', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell', 'Shotgun Shell']
		this_object.unit = get_unit_by_name('Childs')
		
	elif object_name == 'Blair':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.peach
		this_object.description = "Blair"
		this_object.inventory = ['Sniper Rifle', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet', 'Sniper Rifle Bullet']
		this_object.unit = get_unit_by_name('Blair')
		
	elif object_name == 'Copper':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.celadon
		this_object.description = "Scalpel"
		this_object.inventory = ['Scalpel']
		this_object.unit = get_unit_by_name('Copper')
		...
		this_object.inventory = ['Throwing Arm']

	elif object_name == 'Nauls':
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = libtcod.red
		this_object.inventory = ['Locker Key', 'J&B', 'Axe']
		...
		this_object = get_predesigned_object_by_name('Humanoid')
		this_object.foreground_colour = pick_random([libtcod.blue, libtcod.light_blue, libtcod.lighter_blue, libtcod.lightest_blue])
		this_object.char = 'd'
		this_object.size = 2
...
def get_predesigned_object_by_name(predesigned_object_name):
	if predesigned_object_name == 'Generic Object':
		...
		if lab_material == 'Glass':
			this_object.foreground_colour = libtcod.light_cyan
			this_object.components = []
			this_object.char = '<'
		elif lab_material == 'Wood':
			this_object.foreground_colour = libtcod.orange
			this_object.components = []
		elif lab_material == 'Metal':
			this_object.foreground_colour = libtcod.dark_grey
			this_object.components = []
		elif lab_material == 'Flesh':
			this_object.foreground_colour = libtcod.light_red
			this_object.components = []
		elif lab_material == 'Cloth':
			this_object.foreground_colour = libtcod.white
			this_object.components = []
...
def get_unit_by_name(unit_name, real_name = None):
	...
	if unit_name == 'Generic Humanoid':
		...
		this_unit.dialogue.append(614)
	elif unit_name == 'MacReady':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
	elif unit_name == 'Windows':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
	elif unit_name == 'KALO':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
	elif unit_name == 'Blair':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
	elif unit_name == 'Copper':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
	elif unit_name == 'Childs':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
		
	elif unit_name == 'Player':
		this_unit = get_unit_by_name('Generic Humanoid')
		this_unit.name = unit_name
		this_unit.dialogue.append(329)
...
def get_dialogue_by_ID(dialogue_ID):
	...
	if dialogue_ID == 1:
		...
		this_dialogue.feelings_range = [1, 5]
		
	elif dialogue_ID == 400:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="You monster, you're The Thing!  Don't try to fool me!.")
		this_dialogue.feelings_range = [0, 0]
	...
	elif dialogue_ID == 329:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="You're looking quite human today.")
			
	elif dialogue_ID == 500:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue='+You are THE THING -- an alien life form with the ability to consume and mimic other life forms, who for reasons unknown, suddenly and unexpectedly crash landed on planet Earth.  You have now awoken after 100,000 years (at least) trapped in the ice.  You slowly approach Antarctic Research Station Outpost 31, which operates with a small crew of dedicated scientists.')
			
	elif dialogue_ID == 501:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue='+These scientists have recently been warned by inhabitants of a nearby Norweigan research lab of the possibility of an alien life form infiltrating their group.  An explosion in the distance indicates there may be very little remaining of the other lab.')
			
	elif dialogue_ID == 502:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue='+Despite your recent visit with the Norweigans, you remain tired, grumpy, and hungry.  How you proceed from here is up to you...')
			
	elif dialogue_ID == 600:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue='Yeah, fuck you too!')
			
	elif dialogue_ID == 601:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Cheatin' bitch.")
			
	elif dialogue_ID == 602:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Ya see, when a man bleeds, it's just tissue, but blood from one of you Things won't obey when it's attacked. It'll try and survive... crawl away from a hot needle, say.")
			
	elif dialogue_ID == 603:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="I know I'm human. And if you were all these things, then you'd just attack me right now, so some of you are still human. This thing doesn't want to show itself, it wants to hide inside an imitation. It'll fight if it has to, but it's vulnerable out in the open. If it takes us over, then it has no more enemies, nobody left to kill it. And then it's won.")
			
	elif dialogue_ID == 604:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Somebody in this camp ain't what he appears to be. Right now that may be one or two of us. By spring, it could be all of us.")
			
	elif dialogue_ID == 605:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Anybody seen Ali Zoheir? Somebody blew out a fuse in the lab. Lights where out in there for an hour; any ONE of us coulda gotten to him. All right, we gotta find him. Nauls, why don't you come with me and we'll look outside. Yollanda, you and Jayden check the inside.")
			
	elif dialogue_ID == 606:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="When I left yesterday, I turned the lights off.")
...
# Childs
		...
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="I just cannot believe any of this voodoo bullshit.")

	elif dialogue_ID == 607:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="You're gonna have to sleep sometime.")
			
	elif dialogue_ID == 608:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="So, how do we know who's human? If I was an imitation, a perfect imitation, how would you know if it was really me?")
			
	elif dialogue_ID == 609:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Nothing human could have made with back here through this weather without a guide line.")
	...
	elif dialogue_ID == 610:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="You guys think I'M crazy! Well, that's fine! Most of ya don't know what's goin' on around here, but I'm damn well sure SOME of you do!")
			
	elif dialogue_ID == 611:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
	...
	elif dialogue_ID == 612:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Did you notice anything strange about the dog? Anything at all?")
			
	elif dialogue_ID == 613:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="How long were you alone with that dog?")
			
			
# Nauls			
			
			
	elif dialogue_ID == 614:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Which one of you disrespectful men been tossing his dirty drawers in the kitchen trash can, huh? From now, I want my kitchen clean, all right? Germ free!")
			
# Copper
			
	elif dialogue_ID == 615:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="My God, what the hell happened here?")
			
	elif dialogue_ID == 616:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="They're not Swedish. They're Norwegian.")
			
# Windows
			
	elif dialogue_ID == 617:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="You guys gonna listen to Sophie Strand? You gonna let her give the orders? I mean, she could BE one of those THINGS!")
			
	elif dialogue_ID == 618:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Reach anybody? We're a thousand miles from nowhere, man, and it's gonna get a hell of a lot worse before it gets any better!")
			
	elif dialogue_ID == 619:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue="Do you think he's changed into one of those Things?")



	elif dialogue_ID == 999:
		this_dialogue = Dialogue(
			ID=dialogue_ID,
			dialogue='')
...
def generate_player_info_panel():
	...
	supplementary_text.append('#Show Controls')
	supplementary_text.append('Press K')
	supplementary_text.append('#Current Score:')
	supplementary_text.append(str(int(scoreboard.session_total_score)))	
...
class Screen_Console:		
	...
	def create_line_on_screen_object_array(self):
		...
		for line_of_text in self.written_text:
			...
			if line_of_text[0] == '+':
				this_colour = libtcod.light_cyan
				self.all_line_on_screen_consoles.append(Line_On_Screen_Console(x = 2, y = line_count, text = ''))
				line_of_text = line_of_text[1:]
				line_count += 1
...
class Scoreboard:		
	...
	def __init__(self):
		...
		self.VALUE_DOG = 1
		self.VALUE_NAULS = 4
		self.VALUE_MACREADY = 7
		self.VALUE_WINDOWS = 6
		self.VALUE_CHILDS = 8
		self.VALUE_BLAIR = 5
		self.VALUE_COPPER = 3
		...
		self.session_unique_enemies_killed = 0
		self.session_difficulty_level = difficulty_level
		self.session_victory = False
		...
		self.best_session_unique_enemies_killed = 0
		self.best_session_difficulty_level = difficulty_level
		self.best_session_victory = False
		

		self.scalpel_locked = True
		self.gun_locked = True
		self.rifle_locked = True
		self.lab_material_change_locked = True
		self.skin_change_locked = True
		self.flamethrower_locked = True
		self.rocket_locked = True
		self.shotgun_locked = True
		self.fov_locked = True
		self.mega_bomb_locked = True
		self.ammo_locked = True
		self.real_time_locked = True
		self.invincible_locked = True
		
		
		
	def check_for_unlockables(self):
	
		unlocked = []
	
		if self.best_session_total_score >= 20 and self.scalpel_locked:
			self.scalpel_locked = False
			unlocked.append('#Scalpel unlocked!')
			
		if self.best_session_total_score >= 30 and self.gun_locked:
			self.gun_locked = False
			unlocked.append('#Gun unlocked!')
			
		if self.best_session_total_score >= 40 and self.rifle_locked:
			self.rifle_locked = False
			unlocked.append('#Sniper Rifle unlocked!')
			
		if self.best_session_total_score >= 50 and self.flamethrower_locked:
			self.flamethrower_locked = False
			unlocked.append('#Flamethrower unlocked!')
			
		if self.best_session_total_score >= 60 and self.skin_change_locked:
			self.skin_change_locked = False
			unlocked.append('#Change Game Skin unlocked!')
			
		if self.best_session_total_score >= 70 and self.mega_bomb_locked:
			self.mega_bomb_locked = False
			unlocked.append('#Mega Bomb unlocked!')
			
		if self.best_session_total_score >= 80 and self.rocket_locked:
			self.rocket_locked = False
			unlocked.append('#Rocket Launcher unlocked!')
			
		if self.best_session_total_score >= 90 and self.shotgun_locked:
			self.shotgun_locked = False
			unlocked.append('#Shotgun unlocked!')
			
		if self.best_session_total_score >= 100 and self.fov_locked:
			self.fov_locked = False
			unlocked.append('#No FOV mode unlocked!')
			
		if self.best_session_total_score >= 125 and self.lab_material_change_locked:
			self.lab_material_change_locked = False
			unlocked.append('#Change Lab Material unlocked!')
			
		if self.best_session_total_score >= 150 and self.real_time_locked:
			self.real_time_locked = False
			unlocked.append('#Real-Time Gameplay Mode unlocked!')
			
		if self.best_session_total_score >= 175 and self.ammo_locked:
			self.ammo_locked = False
			unlocked.append('#Unlimited Ammo unlocked!')
			
		if self.best_session_total_score >= 200 and self.invincible_locked:
			self.invincible_locked = False
			unlocked.append('#Invinciblity unlocked!')
			
		if unlocked:
			menu_prompt(written_text = unlocked)
			

		
	def humanoid_killed(self, enemy_name, attacker_taking_credit = None):
		global player
		self.session_scientists_killed += 1
		points = self.VALUE_SCIENTIST * difficulty_multiplier()
		if attacker_taking_credit <> player:
			attacker_taking_credit = None
			points = int(points / 3)
		self.session_total_score += (points)
		
		if attacker_taking_credit:
			kill_message = enemy_name + ' has been killed by The Thing! ' + str(points) + ' points awarded!'
		else:
			kill_message = enemy_name + ' has been killed by another entity in the paranoid frenzy, target value reduced, only ' + str(points) + ' points awarded.'
		Message(kill_message, 'Debug')
	
	def dog_killed(self, enemy_name, attacker_taking_credit = None):
		global player
		self.session_dogs_killed += 1
		points = self.VALUE_DOG * difficulty_multiplier()
		if attacker_taking_credit <> player:
			attacker_taking_credit = None
			points = int(points / 3)
		self.session_total_score += (points)
	...
	def unique_enemy_killed(self, enemy_name, attacker_taking_credit = None):
		global player
		self.session_unique_enemies_killed += 1
		if enemy_name == 'MacReady':
			self.session_macready_killed = 1
			
		points = 0
			
		if enemy_name == 'MacReady':
			points = self.VALUE_MACREADY * difficulty_multiplier()
		elif enemy_name == 'Nauls':
			points = self.VALUE_NAULS * difficulty_multiplier()
		elif enemy_name == 'Windows':
			points = self.VALUE_WINDOWS * difficulty_multiplier()
		elif enemy_name == 'Blair':
			points = self.VALUE_BLAIR * difficulty_multiplier()
		elif enemy_name == 'Childs':
			points = self.VALUE_CHILDS * difficulty_multiplier()
		elif enemy_name == 'Copper':
			points = self.VALUE_COPPER * difficulty_multiplier()
	...
	def reset_session(self, name = None):
		if not name:
			name = 'Player Name'
	...
	def calculate_bonuses(self):
		...
		self.session_total_score -= int(self.session_turns_taken / 50)
		if self.session_total_score < 0: self.session_total_score = 0
		final_message.append('---')
		final_message.append('Turns taken penalty: ' + str(int(self.session_turns_taken / 50)))
		final_message.append('---')
			
		if self.session_macready_killed:
			self.session_total_score = int(self.session_total_score * 1.1)
			final_message.append('MacReady killed!  10 percent bonus!')
			final_message.append('---')
			
		if self.session_victory:
			self.session_total_score = int(self.session_total_score * 1.25)
			final_message.append('All units killed!  25 percent bonus!')
			final_message.append('---')
			
		menu_prompt(written_text = final_message, header = 'Final Score Adjustments')
	...
	def add_session_data_to_total(self):
		if self.session_total_score >= self.best_session_total_score:
			...
			self.best_session_unique_enemies_killed = self.session_unique_enemies_killed
			self.best_session_difficulty_level = difficulty_level
			self.best_session_macready_killed = self.session_macready_killed
			self.best_session_victory = self.session_victory

	def return_session_scores_as_text_array(self):
		score_data_array = []
		score_data_array.append('#Final Score')
		score_data_array.append('')
		score_data_array.append('Name:                       ' + self.session_name)
		score_data_array.append('Difficulty:                 ' + self.session_difficulty_level)
		...
		score_data_array.append('Unique Enemies Killed       ' + str(self.session_unique_enemies_killed))
		score_data_array.append('Time Penalty:               ' + str(int(0 - (self.session_turns_taken / 50))))
		score_data_array.append('MacReady Killed:            ' + str(boolean_to_yes_no(self.session_macready_killed)))
		score_data_array.append('Game Victory                ' + str(boolean_to_yes_no(self.session_victory)))
		score_data_array.append('')
	...
	def return_all_scores_as_text_array(self):
		...
		score_data_array.append('Difficulty:                 ' + self.best_session_difficulty_level)
		...
		score_data_array.append('Unique Enemies Killed       ' + str(self.best_session_unique_enemies_killed))
		score_data_array.append('Time Penalty:               ' + str(int(0 - (self.best_session_turns_taken / 50))))
		score_data_array.append('MacReady Killed:            ' + str(boolean_to_yes_no(self.best_session_macready_killed)))
		score_data_array.append('Game Victory                ' + str(boolean_to_yes_no(self.session_victory)))
		score_data_array.append('')
...
def render_all(exclude_tooltips = False):
	...
	global global_render_tooltips
	
	if not global_render_tooltips:
	...
	if not player:
		...
		if fov_recompute:
			...
			"""
			if not SHOW_ALL:
				y_start = normalize_to_map_height(player.y - MAX_VISIBILITY_RADIUS)
				y_end = normalize_to_map_height(player.y + MAX_VISIBILITY_RADIUS)
				x_start = normalize_to_map_width(player.x - MAX_VISIBILITY_RADIUS)
				x_end = normalize_to_map_width(player.x + MAX_VISIBILITY_RADIUS)
			else:
				y_start = 0
				y_end = MAP_HEIGHT
				x_start = 0
...
def menu_prompt(options = [], written_text = [], header = None, prompt_for_choice = None, render_screen_before_displaying = True, main_screen_console = False, click_sound = None):
	global menu_click
	
	if not click_sound:
		click_sound = menu_click
...
def main_menu():
	global SHOW_ALL, HEAR_ALL, PLAY_MUSIC, fov_recompute, scoreboard, main_game_ui, lab_material, difficulty_level
	...
	global INVINCIBLE_ACTIVE
	global REAL_TIME_ACTIVE
	global NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME
	
	global cheats_on
	
	menu_noise_toggle = True
	...
	while not libtcod.console_is_window_closed():
		...
		fov_locked = '{Locked} No FOV Mode'
		
		skin_change_locked = '{Locked} Change Game Skin'
		
		lab_material_locked = '{Locked} Change Lab Material'
		
		real_time_locked = '{Locked} Real-Time Mode'
		
		if not scoreboard.fov_locked:
			fov_locked = 'No FOV Mode [' + boolean_to_yes_no(SHOW_ALL) + ']'
			
		if not scoreboard.lab_material_change_locked:
			lab_material_locked = 'Change Lab Material'
			
		if not scoreboard.skin_change_locked:
			skin_change_locked = 'Change Game Skin'
			
		if not scoreboard.real_time_locked:
		
			if NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME == 1:
				slow_of_fast = ''
			else:
				slow_of_fast = '{Fast}'
				
			real_time_locked = 'Real-Time Mode [' + boolean_to_yes_no(REAL_TIME_ACTIVE) + '] ' + slow_of_fast
		
		menu_options = ['Play a new game', 'Screen Resolution', 'Toggle Fullscreen', 'Difficulty Level', 'Scoreboard', 'Reset Scoreboard', 'Cheats', fov_locked, lab_material_locked, skin_change_locked, real_time_locked, 'Quit']
		
		if not stay_in_menu:
			choice = menu_prompt(options = menu_options, written_text = [get_dialogue_by_ID(500), get_dialogue_by_ID(501), get_dialogue_by_ID(502)], header = 'THE THING', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
		...
		if choice == menu_options[0]:  # new game
			...
			'800 x 600',
			'Cancel']
			res_choice = menu_prompt(options = res_options, written_text = ['Choose your screen resolution:'], header = 'Screen Resolution', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			...
			difficulty_options = ['Easy', 'Normal', 'Hard', 'Very Hard', 'Insane']
			difficulty_level = menu_prompt(options = difficulty_options, written_text = ['Choose your difficulty level:'], header = 'Difficulty Level', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
				
		elif choice == menu_options[4]:
			scoreboard.show_scoreboard()
			
		elif choice == menu_options[5]:
			are_you_sure = menu_prompt(options = ['Yes', 'No'], written_text = ['Are you sure?'], header = 'Confirm', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			if are_you_sure == 'Yes':
				# reset scorebaord entirely
				scoreboard = Scoreboard()
				save_scoreboard()
			...
			if not scoreboard.scalpel_locked:
				cheat_options.append('Scalpel [' + boolean_to_yes_no(SCALPEL_ACTIVE) + ']')
			if not scoreboard.gun_locked:
				cheat_options.append('Gun [' + boolean_to_yes_no(GUN_ACTIVE) + ']')
			if not scoreboard.rifle_locked:
				cheat_options.append('Sniper Rifle [' + boolean_to_yes_no(RIFLE_ACTIVE) + ']')
			if not scoreboard.flamethrower_locked:
				cheat_options.append('Flamethrower [' + boolean_to_yes_no(FLAMETHROWER_ACTIVE) + ']')
			if not scoreboard.rocket_locked:
				cheat_options.append('Rocket Launcher [' + boolean_to_yes_no(ROCKET_ACTIVE) + ']')
			if not scoreboard.shotgun_locked:
				cheat_options.append('Shotgun [' + boolean_to_yes_no(SHOTGUN_ACTIVE) + ']')
			if not scoreboard.mega_bomb_locked:
				cheat_options.append('Mega Bomb [' + boolean_to_yes_no(MEGA_BOMB_ACTIVE) + ']')
			if not scoreboard.ammo_locked:
				cheat_options.append('Unlimited Ammo [' + boolean_to_yes_no(UNLIMITED_AMMO_ACTIVE) + ']')
			if not scoreboard.invincible_locked:
				cheat_options.append('Invincibility [' + boolean_to_yes_no(INVINCIBLE_ACTIVE) + ']')
			cheat_options.append('Done')

			cheat_choice = menu_prompt(options = cheat_options, written_text = ['#Choose your bonuses:', ''], header = 'Cheats', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			
			if not cheat_choice or 'Done' in cheat_choice:
				stay_in_menu = False
			else:
				stay_in_menu = True
				if 'Scalpel' in cheat_choice:
					if SCALPEL_ACTIVE:
						SCALPEL_ACTIVE = False
					else:
						SCALPEL_ACTIVE = True
				elif 'Gun' in cheat_choice:
					if GUN_ACTIVE:
						GUN_ACTIVE = False
					else:
						GUN_ACTIVE = True
				elif 'Sniper Rifle' in cheat_choice:
					if RIFLE_ACTIVE:
						RIFLE_ACTIVE = False
					else:
						RIFLE_ACTIVE = True
				elif 'Flamethrower' in cheat_choice:
					if FLAMETHROWER_ACTIVE:
						FLAMETHROWER_ACTIVE = False
					else:
						FLAMETHROWER_ACTIVE = True
				elif 'Rocket Launcher' in cheat_choice:
					if ROCKET_ACTIVE:
						ROCKET_ACTIVE = False
					else:
						ROCKET_ACTIVE = True
				elif 'Shotgun' in cheat_choice:
					if SHOTGUN_ACTIVE:
						SHOTGUN_ACTIVE = False
					else:
						SHOTGUN_ACTIVE = True
				elif 'Mega Bomb' in cheat_choice:
					if MEGA_BOMB_ACTIVE:
						MEGA_BOMB_ACTIVE = False
					else:
						MEGA_BOMB_ACTIVE = True
				elif 'Unlimited Ammo' in cheat_choice:
					if UNLIMITED_AMMO_ACTIVE:
						UNLIMITED_AMMO_ACTIVE = False
					else:
						SCALPEL_ACTIVE = False
				...
				elif 'Invincibility' in cheat_choice:
					if INVINCIBLE_ACTIVE:
						INVINCIBLE_ACTIVE = False
					else:
			...
			if not stay_in_menu:
				if not SCALPEL_ACTIVE and not GUN_ACTIVE and not FLAMETHROWER_ACTIVE and not ROCKET_ACTIVE and not SHOTGUN_ACTIVE and not MEGA_BOMB_ACTIVE and not UNLIMITED_AMMO_ACTIVE and not RIFLE_ACTIVE and not INVINCIBLE_ACTIVE:
					cheats_on = False
					menu_prompt(written_text = "Cheats disabled.  Scores will be recorded.", header = 'Cheats', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
				else:
					cheats_on = True
					menu_prompt(written_text = "Cheats enabled.  Scores will not be recorded.", header = 'Cheats', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			...
			if not scoreboard.fov_locked:
				...
				menu_prompt(written_text = "Locked", header = 'Locked', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
				
		elif choice == menu_options[8]:
			if not scoreboard.lab_material_change_locked:
				mat_choice = ['Stone', 'Metal', 'Wood', 'Glass', 'Cloth', 'Flesh']
				lab_material = menu_prompt(options = mat_choice, written_text = ['Choose your lab material:'], header = 'Lab Material', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			else:
				menu_prompt(written_text = "Locked", header = 'Locked', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			...
			if not scoreboard.skin_change_locked:
				...
				skin_choice = menu_prompt(options = skin_options, written_text = ['Choose your skin:'], header = 'Screen Resolution', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			...
			if not scoreboard.real_time_locked:
				if NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME == 1 and REAL_TIME_ACTIVE:
					slow_of_fast = '{Fast}'
					NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME = 0.5
					REAL_TIME_ACTIVE = True
				elif NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME == 0.5 and REAL_TIME_ACTIVE:
					slow_of_fast = ''
					NUMBER_OF_SECONDS_BETWEEN_TURNS_REAL_TIME = 1
					REAL_TIME_ACTIVE = False
				else:
		...
		elif choice == menu_options[11] or not choice:  # quit
			are_you_sure = menu_prompt(options = ['Yes', 'No'], written_text = ['Are you sure you want to quit?'], header = 'Confirm', render_screen_before_displaying = False, main_screen_console = True, click_sound = menu_noise_2)
			if are_you_sure == 'Yes':
				sys.exit()
